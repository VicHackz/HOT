...
export function Input({ ...rest }) { return <input className="control" {...rest} /> }
export function Textarea({ rows = 3, ...rest }) { return <textarea className="control" rows={rows} {...rest} /> }
export function Select({ children, ...rest }) { return <select className="control" {...rest}>{children}</select> }
...
export function Swatch({ seed, label, h = 160 }) {
    ...
    <div style={{ position: 'fixed', right: 24, top: 24, display: 'flex', flexDirection: 'column', gap: 8, zIndex: 200 }}>
      {toasts.map(t => (
        <div key={t.id} onClick={() => removeToast(t.id)}
          ...
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: t.kind === 'success' ? 'var(--sage)' : t.kind === 'error' ? 'var(--crimson)' : 'var(--ochre)' }} />
          {t.msg}
        </div>
      ))}
    </div>
